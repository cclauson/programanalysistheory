...
\path (A) edge [] node[] {} (C);
\end{tikzpicture}
\caption{\texttt{HALT}-node} \label{fig:M1}
\end{center}
...
A \emph{program control graph} is defined as a directed graph with three kinds of nodes with the following properties:
...
(Define TRS-based evaluation semantics here)
