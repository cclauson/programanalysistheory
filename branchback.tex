...
\begin{substopenness}
If $p$ is a program formula open in $L_{O}$, $l$ is in $L_{O}$, and $q$ is a program formula not open in $l$, then $[\texttt{branchback(}l\texttt{)} \mapsto q]p$ is not open in $l$.
\end{substopenness}
\begin{proof}
(TODO: Prove this)
\end{proof}
...
(TODO: Finish this proof, possibly change previous theorem to appropriately support)
\end{proof}
